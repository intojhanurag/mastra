...
Both Server and Studio deploy require `MASTRA_ORG_ID` and `MASTRA_PROJECT_ID` as env vars in headless mode, because API tokens cannot list organizations:
    ...
    steps:
      ...
      - uses: actions/setup-node@v4
        with:
          node-version: '22'
      - run: pnpm install
      - run: pnpm mastra studio deploy --yes
        env:
          MASTRA_API_TOKEN: ${{ secrets.MASTRA_API_TOKEN }}
          MASTRA_ORG_ID: ${{ secrets.MASTRA_ORG_ID }}
          MASTRA_PROJECT_ID: ${{ secrets.MASTRA_PROJECT_ID }}
